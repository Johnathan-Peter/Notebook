...
\end{cases} $$
其中：$ f(x) = -\frac{1}{I_{xx}} (k_1 x_2 + k_2 x_2^3), \quad g = \frac{1}{I_{xx}}, \quad d_1(t) = \frac{1}{I_{xx}} d_\phi(t) $
...
采用经典的PID控制器，控制律为：
$ u_\phi = K_p e + K_i \int_0^t e(\tau) d\tau + K_d \frac{de}{dt}$$其中$ e = \phi_{\text{ref}} - \phi $。参数通过Matlab自带的PID参数调节功能自整定，并作为性能对比基准。
